...
var AdminView = function(model) {
    ...
    this.selectiveCourseCatalogSelected = function($element) {
        ...
        $element.parents('.formTable').find('.course-list .selectCourse option').prop('selected', false);
        self.bindevent();

        var subscourselist = self.model.get('_domain').SelectiveBoxReset == true ? [] : self.model.get('_domain').data.subscribedcourselist;
    ...
    this.setSelectiveCourseRetrieved = function(valueArray) {
        ...
        //model set to [] to remove any regression
    ...
    this.courseSelectedAction = function($element) {
        ...
        if (totalElements == selectedElements) {
            $element.parents('.formTable').find('.entire-catalog').prop('checked', true);
            $element.parents('.formTable').find('.selective-course').prop('checked', false);
        }

        $element.addClass('newEle');
        if (self.model.get('isSchoolPopup') === false && totalElements === selectedElements) {
            $element.parents('.formTable').find('.entire-catalog').prop('checked', true);
            $element.parents('.formTable').find('.selective-course').prop('checked', false);
        } else if (self.model.get('isSchoolPopup') === true && totalElements === selectedElements) {
            self.model.set('selectedAllSclCourse', true);
            if (self.model.get('selectedAllSclCatalogs') && self.model.get('selectedAllSclCourse')) {
                $element.parents('.formTable').find('.entire-catalog').prop('checked', true);
                $element.parents('.formTable').find('.selective-course').prop('checked', false);
            }
        }
    ...
    this.courseUnSelectedAction = function($element) {
        ...
        while (selectedCourses.indexOf(dataCourseId) > -1) {
            ...
            $element.parents('.formTable').find('.entire-catalog').prop('checked', false);
            $element.parents('.formTable').find('.selective-course').prop('checked', true);
        }
        self.model.set('selectedCourses', selectedCourses);
    };
    ...
    this.courseCatalogsSelected = function($element) {
        var localLength,
            ...
            i;
        $element.addClass('newEle');
        if (self.model.get('isSchoolPopup') === true && totalElements === selectedElements) {
            self.model.set('selectedAllSclCatalogs', true);
            if (self.model.get('selectedAllSclCatalogs') && self.model.get('selectedAllSclCourse')) {
//                $element.parents('.formTable').find('.entire-catalog').prop('checked', true);
//                $element.parents('.formTable').find('.selective-course').prop('checked', false);
            }
    ...
    this.editSchoolData = function(rslt, parametersObj) {
        ...
        if (rslt.messageType === "SUCCESS") {
            ...
            //Bydefault for editing school,the entire-catalog will be selected
            //if (rslt.data.customization.edivatelearn.fullsubscription) {
//            $('.editschlpopupBox').find('.entire-catalog').prop('checked', 'checked');
//            //} else {
//            $('.editschlpopupBox').find('.selective-course').prop('disabled', "disabled");
            //}
            //update the course list of a catalog when add of a school domain is called
            // $.each(rslt.data.subscribedproviderlist, function(index, val) {
    ...
    this.updateCourseData = function(res, parametersObj) {
        ...
        for (loopVar = 0; loopVar < _selectedCatalogs.length; loopVar++) {
            if (_selectedCatalogs[loopVar] != self.model.get('_customCatalogId')) {
                totalCourses = totalCourses.concat(_allCoorsesList[_selectedCatalogs[loopVar]]);
            }
        } */
    ...
    this.editDomainFromData = function(res, parametersObj) {
        ...
        // if(res.data.subscribedproviderlist){
        //     for(loopVar=0;loopVar<res.data.subscribedproviderlist.length;loopVar++){
    ...
    this.resetDomainVar = function(res) {
        if (self.model.get('_domain')) {
        	self.model.set('_domain',{})
        }
        self.model.set('_domain',res)
    };
    ...
    this.schoolPopupDataCall = function(res, parametersObj) {
        ...
        if (res.messageType === "SUCCESS") {
            ...
            //Bydefault for adding scholl,the entire-catalog will be selected
            //if (res.data.customization.edivatelearn.fullsubscription) {
            $('.addschlpopupBox').find('.entire-catalog').prop('checked', 'checked');
            //} else {
            //$('.addschlpopupBox').find('.selective-course').prop('disabled', 'disabled');
            //}
    ...
    this.populateCatalogsCourses = function(resultdistrict, type) {
        ...
        // self.model.set('allCatalogsList', subscribedproviderlist);
        // self.model.set('_allCoorsesList', subscribedcourselist);
        ...
        for (loopVar = 0; loopVar < allCatalogs.length; loopVar++) {
            if (subscribedproviderlist && subscribedproviderlist.indexOf(allCatalogs[loopVar].id) > -1) {
                ...
                //courseListURI = self.model.get('_config').courseList + "/" + allCatalogs[loopVar].id + "/" + districtDomain;
                ...
                if (type === 'edit') {
                    popup.find('.entire-catalog').prop('checked', false);
                    popup.find('.selective-course').prop('checked', true).removeAttr('disabled', false);
                }
            }
            // commented out as now only the providerList need to be
            // displayed in the dropdown
            //             else {
            //             html += '<option id="checkbox' + loopVar + '" type="checkBox"
            //             data-domain-id="' + allCatalogs[loopVar].id + '"
            //             data-course-list="">' + allCatalogs[loopVar].name + '</option>';
            //             }
        }

        //Moved out as the list of total courses can be retrieved from district itself
    ...
    this.addCourseListData = function(districtData, parametersObj) {
        ...
        self.setEntireSelectiveRadioButton(districtData.data.subscribedcourselist,parametersObj.schoolDomain.data.subscribedcourselist,selector);
        self.cursordefault();
        $('.popGrayBg').show();
        self.bindevent();
    };

    this.setEntireSelectiveRadioButton=function(originalList,selectedList,selector){
    	if(originalList.length==selectedList.length){
    		  $('.'+selector).find('.entire-catalog').prop('checked', true);
    		  $('.'+selector).find('.selective-course').prop('checked', false);
    	}
    	else
    		{
    		 $('.'+selector).find('.entire-catalog').prop('checked', false);
   		  	 $('.'+selector).find('.selective-course').prop('checked', true)
    		}
    };
    //NKN
